---
layout: home
lang: ko
nav: home
title: HOME
lang_ko: /ko/
lang_en: /en/
lang_ja: /ja/
person_name: Iheon Kim
intro: 이 사이트는 이론물리 연구, 강의와 세미나 노트, 그리고 서브컬쳐에 대한 비평과 기록을 모아두는 개인 아카이브다.
cv:
- label: AFFILIATION
  value: POSTECH, Department of Physics
- label: POSITION
  value: PhD Student
- label: PhD ADVISOR
  value: Professor Hee-Cheol Kim
- label: RESEARCH
  value: Gauge theory / quantum field theory / lattice models / neutral-atom simulation
    / automorphic forms
- label: INTERESTS
  value: Anime / games / music / subculture criticism
- label: CV
  value: '<a href="/assets/files/cv.pdf">PDF</a> (준비중)'
- label: CONTACT
  value: '<a href="mailto:2heon2@postech.ac.kr">2heon2@postech.ac.kr</a>'
after_text:
- 물리학 글은 연구 메모, 세미나 발표 정리, 강의 노트, 참고문헌 정리 등으로 나뉜다.
- 서브컬쳐 글은 애니메이션, 게임, 음악, 아이돌, 라이브하우스 문화에 대한 비평과 에세이를 포함한다.
---
